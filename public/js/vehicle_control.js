...
//v pc verzii budu siabled a ifmobile device, tak budu enabled//toto osetri ale v indexe vozidla
    ...
    tick: function() {
      ...
      if(vehicleFrontMove == false && acceleration >= Math.sin(0.05)){
        
          acceleration -= 0.1 / 300;  
          steeringVehicle(false);
      }    

        if(acceleration <= - Math.sin(0.03)){
          acceleration += Math.sin(0.1 / 200);    
          steeringVehicle(true);
        }
...
function steeringVehicle(isfront){
  if(isfront){    
  ...
  if(!isfront){
          if(vehicleLeftMove == true){   
             if(steering < 0.009 && steering > -0.01) {
                steering -= 0.01 / 10;  
             }
         };
